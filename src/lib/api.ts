...
const API_BASE_URL = 'https://www.app.justcbt.com.ng/';
...
api.interceptors.request.use((config) => {
  if (typeof window !== 'undefined') {
    const token = getCookie('cbt_access'); // Get access token from cookies
    if (token) {
  ...
  async (error) => {
    ...
    if (response && response.status === 401 && !response.config._retry) {
      ...
      try {
        ...
        const refreshToken = getCookie('cbt_refresh');
        ...
        setCookie('cbt_access', newAccessToken, { path: '/' });
